<template>
    <div>
        ...
        <div v-for="(a,index) in exam" 
            v-bind:key="index"
            style="display:flex; direction:row; margin-bottom: 10px;justify-content: center; margin: 5px auto"
        >
            <div style=" color:white;width: 100px;text-align:right">
                {{a.numberA}} x {{a.numberB}} = 
            </div>
            <input :id="index" v-model="results[index]"/>

        </div>

        <button @click="correct">Corregir</button>
